// src/modules/finance/pages/ProfilePage.tsx
...
import { useAppDispatch, useAppSelector, F, DATE, INR, fieldSx, labelSx, selSx } from '../hooks';
import {
  fetchProfile,
  doUpdateProfile,
  doUploadAvatar,
...
function EditProfileDialog({ open, profile, onClose }: { open: boolean; profile: any; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth
      ...
      <DialogTitle sx={{ color: F.text, fontWeight: 700, fontSize: 15, pb: 1 }}>Edit Profile</DialogTitle>
      <DialogContent dividers sx={{ borderTop: `1px solid ${F.border}`, borderBottom: `1px solid ${F.border}` }}>
        <Grid container spacing={2} mt={0.25}>
          ...
          <Grid item xs={12}>
            <TextField fullWidth size="small" label="Email" value={editedProfile.email}
              ...
              FormHelperTextProps={{ sx: { color: F.textSub, fontSize: 10 } }} />
          </Grid>
          <Grid item xs={12}>
          ...
          <Grid item xs={6}>
            <Box sx={{ p: 1, border: `1px dashed ${F.border}`, borderRadius: '8px', bgcolor: 'rgba(255,255,255,0.03)' }}>
              <Typography sx={{ color: F.textSub, fontSize: 10, textTransform: 'uppercase', mb: 0.5 }}>Aadhar Card</Typography>
              <Typography sx={{ color: F.text, fontSize: 13, fontWeight: 600 }}>{editedProfile.aadhar || 'Not Provided'}</Typography>
            </Box>
          </Grid>
          <Grid item xs={6}>
            <Box sx={{ p: 1, border: `1px dashed ${F.border}`, borderRadius: '8px', bgcolor: 'rgba(255,255,255,0.03)' }}>
              <Typography sx={{ color: F.textSub, fontSize: 10, textTransform: 'uppercase', mb: 0.5 }}>PAN Card</Typography>
              <Typography sx={{ color: F.text, fontSize: 13, fontWeight: 600 }}>{editedProfile.pan || 'Not Provided'}</Typography>
            </Box>
          </Grid>
          <Grid item xs={6}>
          ...
          <Grid item xs={12}>
            <TextField fullWidth size="small" label="Skills & Competencies (comma separated)" value={editedProfile.skills}
              ...
              sx={fieldSx} InputLabelProps={{ sx: labelSx }}
              helperText="e.g. Accounting, Tax, Audit"
      ...
      <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
        <Button onClick={onClose} sx={{ color: F.textSub, textTransform: 'none', fontSize: 13 }}>Cancel</Button>
...
export default function ProfilePage() {
  const dispatch = useAppDispatch();
  const { profile } = useAppSelector((s: any) => s.finance);
        ...
        <Grid item xs={12} md={4}>
          <Card>
            <Box p={2.5} textAlign="center">
              ...
              <Typography sx={{ color: F.text, fontWeight: 700, fontSize: 17 }}>{profile?.name ?? '—'}</Typography>
              <Typography sx={{ color: F.textSub, fontSize: 13 }}>{ap?.designation ?? 'Finance Professional'}</Typography>
              {ap?.department && (
                <Chip label={ap.department} size="small"
                  sx={{ mt: 1, bgcolor: `${F.primary}15`, color: F.primary, fontSize: 11 }} />
              )}

              {ap?.bio && (
                <Typography sx={{ color: F.textSub, fontSize: 12.5, mt: 2, lineHeight: 1.6, textAlign: 'left' }}>
                  {ap.bio}
                </Typography>
              )}
              ...
              {ap?.skills?.length > 0 && (
                <Box mt={2} textAlign="left">
                  <Typography sx={{ color: F.muted, fontSize: 11, mb: 0.75, textTransform: 'uppercase', letterSpacing: 0.4 }}>Skills & Competencies</Typography>
                  <Box display="flex" flexWrap="wrap" gap={0.5}>
                    {ap.skills.map((s: string) => (
                      <Chip key={s} label={s} size="small"
                        sx={{ fontSize: 10.5, height: 22, bgcolor: `${F.primary}15`, color: F.primary }} />
                    ))}
                  </Box>
                </Box>
              )}
              ...
              {ap?.languagesSpoken?.length > 0 && (
                <Box mt={2} textAlign="left">
                  <Typography sx={{ color: F.muted, fontSize: 11, mb: 0.75, textTransform: 'uppercase', letterSpacing: 0.4 }}>Languages</Typography>
                  <Box display="flex" flexWrap="wrap" gap={0.5}>
                    {ap.languagesSpoken.map((l: string) => (
                      <Chip key={l} label={l} size="small"
                        sx={{ fontSize: 10.5, height: 22, bgcolor: `${F.green}12`, color: F.green }} />
                    ))}
                  </Box>
                </Box>
              )}
